...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-200 ease-out ${
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {items.map((item) => (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-3 md:py-2 rounded-lg text-sm font-medium transition-colors ${
                  isActive
